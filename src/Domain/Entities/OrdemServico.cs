...
public class OrdemServico : BaseEntity
{
    ...
    public void Iniciar(Guid? usuarioId)
    {
        if (Status != StatusOrdemServico.AguardandoExecucao)
            throw new DomainException("A ordem de serviço não está aguardando início.");
    ...
    public void AtualizarStatus(Guid? usuarioId)
    {
        ...
        if (_servicos.Any(x => x.Status == StatusServico.EmExecucao))
        {
            Status = StatusOrdemServico.EmExecucao;
        ...
        else if (_servicos.Count != 0 && _servicos.All(x => x.Status == StatusServico.Finalizada))
        {
    ...
    public void Entregar(Guid? usuarioId)
    {
        if (Status != StatusOrdemServico.Finalizada)
            throw new DomainException("A ordem de serviço não está finalizada.");
    ...
    public void CarregarItens(IEnumerable<OrdemServicoItemEstoque> itens)
    {
        _itens.Clear();
        _itens.AddRange(itens);
    }

    public void CarregarServicos(IEnumerable<OrdemServicoServico> servicos)
    {
        _servicos.Clear();
